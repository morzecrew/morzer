...
Indexed by [Repowise](https://repowise.dev). Last indexed: 2026-09-17 (commit 75569e8). Confidence: 100%.
...
- `internal/adapters/backup/hookbackup/backup.go` — 15 bug fixes, last fix 6 weeks ago (bug magnet); 23 commits/90d
- `internal/adapters/target/sftp/sftp.go` — 15 bug fixes, last fix 6 weeks ago (bug magnet); 20 commits/90d
- `internal/lifecycle/ops/update.go` — 12 bug fixes, last fix 3 weeks ago (bug magnet); 25 commits/90d
- `internal/cli/root.go` — 12 bug fixes, last fix 6 weeks ago (bug magnet); 40 commits/90d
- `internal/cli/commands.go` — 11 bug fixes, last fix 4 weeks ago (bug magnet); 29 commits/90d

### Code health
Three co-equal signals: defect risk 7.24/10 avg, hotspot health 5.18/10 (stable), worst `.agents/skills/pr-review-loop/scripts/pr_loop.py` at 1.0/10 · maintainability 9.23/10 · performance risk 216 open static I/O-in-loop / N+1 findings. Detail: `get_health()`.

Critical files:
- `internal/infra/lock/lock_test.go` — change entropy — impact −2.6
- `rfcs/0008-test-coverage-program.md` — change entropy — impact −2.3
- `internal/cli/prompt_test.go` — churn risk — impact −2.0
- `schemas/selfhost-v1alpha1-manifest.json` — untested hotspot — impact −2.0
- `rfcs/INDEX.md` — untested hotspot — impact −2.0
