...
| `--dry-run` | Plan only; make no changes. Prints the step list and, where applicable, a configuration diff. A plan validates the release it plans against and refuses one the operation would refuse; where it could not check the bundle — a remote reference is not fetched for a plan — it says so. |
