...
- **Status:** ✅ Complete — P1 shipped 2026-08-08 (the two-pass decode), P2 on
  2026-08-09 (the five fields) and P3 the same day
  (`bundle.uncompressed_size`), alongside the
  [0011](0011-bundled-container-images.md) P1 read that gives it a purpose. Unresolved questions 2 and 3 are resolved into
...
| 13 | A `min_manager_version` floor is **not enforced against a manager whose own version carries the prerelease `git describe --tags` produces between tags** — `<N>-g<sha>`, optionally `-dirty` | That stamp is *derived*: it names the **last** tag, so the build that first understands a new field reports itself as a prerelease of the release *before* the one shipping it, and it understates itself for the whole development cycle. Measured on this tree: the build that added `runtimes:` called itself `0.2.0-9-g8c5a81c`, which semver orders below the `0.3.0` floor its own scaffold writes — so the shipped binary refused the bundle `release new` had just written and reported it as "a bug in the scaffold". Deliberately the exact shape rather than "any prerelease": `rc.1` is the different claim *this build really is older*, which a floor must still be allowed to refuse, and a dirty tree at a tag (`0.3.0-dirty`) does not match and stays held to the floor. **Known hole, named rather than papered over:** a build from an *older* branch is stamped the same way — `0.1.0-5-gabc1234` — and is genuinely behind rather than ahead, so it takes the exemption it should not have. `git describe` cannot tell the two apart: both are "N commits past some tag", and which tag is the last one is the whole question. The blast radius is bounded to builds from source, since a released binary sits on a clean tag and is held to the floor; the cost there is a manifest that parses being accepted by a build that lacks the behaviour its floor was raised for. Closing it needs a version that is *declared* rather than derived — the next release a build is working toward, stamped at build time — which is a change to how this project versions itself and not to this check. `ASSUMED`, not locked — it accommodates one version-stamping scheme, and should fall rather than be defended if the project ever stamps differently. Consequence: a developer on an untagged build gets the strict decoder's unknown-field error instead of the clearer upgrade message, which is the trade this function already makes wherever it cannot answer honestly. Added by execution 2026-08-18 — see EXECUTION-LOG.md D-023, narrowed by PR #53's review from the wider form that entry proposes. |
