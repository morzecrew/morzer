...
parameter file is therefore referenced **without** the `-` prefix, and the hard
problem is the ordering rather than the persistence: under Compose the product
...
| 21 | The parameter `EnvironmentFile` is referenced **without** the `-` prefix, and any unit that reads it is ordered behind whatever renders it | LOCKED | Measured, twice, in a booted venue (§12 item 4). A tmpfs is empty at every boot, so the file is always absent when the transaction begins and "does it survive a reboot" was the wrong question. What is left is a choice between two failure modes. Without the prefix an unordered unit fails and says why: `Failed to load environment files`, `Result=resources`. With it, the unit **starts, reports success, and runs with the parameter empty** -- measured as `B_PORT=[]` on a unit systemd marked active, having started and finished before the render unit wrote the file. Refused: the `-` prefix as a convenience for first boot, because the state it produces is a product running on empty configuration that every health surface reports as fine, and nothing downstream can tell it from a correct start. LOCKED rather than ASSUMED: it is the difference between a deployment that refuses and one that lies, and the cost of reopening it is discovering the difference in production. Consequence: P3's generated units must carry an explicit dependency on the render, which Compose gets for free because its product unit's `ExecStart` *is* `apply --startup`. Also note `ExecMainStatus=0` on the failing unit -- nothing ran, so there is no exit code, and a check reading exit status alone sees a zero from a unit that failed. Added by execution 2026-08-18 -- see EXECUTION-LOG.md D-038. |
...
4. **Whether an `EnvironmentFile` on tmpfs can be read by a unit at boot** (§4.3).
   **Measured 2026-08-18. Both halves, and the answer is that the file's
   ...
   environment does not have; the substitute runs the same systemd, mounts
   `/run` as a real tmpfs (`rw,nosuid,nodev,noexec,relatime,mode=755`) and
   executes a real boot transaction, which is what the ordering question needs.
   ...
   **The ordering half**, measured across two independent boots of the venue.
   ...
   **What this settles.** A tmpfs is empty at every boot by definition, so the
   ...
   product comes up misconfigured and systemd reports success, which is the
   one outcome nothing downstream can detect.
   ...
   **Consequence for §4.3, and it is not what the section feared.** §4.3 called
   ...
   generator, with no natural ordering behind the manager. P3 owes that graph an
   explicit dependency, and owes it without the `-` prefix. Neither is a
   discovery this item can make; both are now measured constraints on it.
