...
| [0004](0004-distribution-and-verification.md) | Distribution and verification | 🚧 In progress — P1–P3 shipped | A source registry dispatches on reference scheme; bundles install from a `tar.zst` archive as readily as a directory, and hash identically either way, so pinning a digest does not pin a transport. Archive extraction refuses traversal, links, device nodes and bombs, each covered by a fixture built in the test that refuses it. minisign verification makes `require_signature` a working control rather than an unconditional failure. HTTPS, OCI, goreleaser and the generated JSON Schema remain. |
| [0005](0005-continuous-integration-and-release.md) | Continuous integration and release | 🚧 In progress | Adds `.github/` with a change-gated `quality → test → coverage → acceptance` topology ported from forze, invoking `just` recipes so local and CI cannot drift. P1–P4 shipped: quality, test, the skip guard, a 45% coverage floor, supply-chain scanning and governance files. Release automation and the acceptance run remain. Fails the build when a real-adapter contract suite *skips*, which today lets CI pass without ever exercising the production secret store. Tagged releases call CI as a reusable workflow; actions are SHA-pinned. |
