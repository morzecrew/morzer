...
| [0007](0007-operator-parameters.md) | Operator parameters | 🚧 In progress | A release declares the knobs an operator may set — ports above all — typed, validated, and delivered to Compose, templates and hooks by one namespaced mechanism, with `morzer config` to set them as a journaled operation. Written after finding that an operator can change *nothing* post-`init`: `installation.yaml` says "values here override release defaults" and is never read back, `Installation.Settings` reaches templates but no flag sets it, and the example bundle's own `${DEMO_HTTP_PORT}` moves the published port while `requirements.ports` and the health URL stay literal — so using it fails `apply` at the health check. Also adds the three-tier example that proves the mechanism and the gate for the undocumented Compose variable ABI. P1-P4 shipped 2026-08-04: a three-tier example (frontend, backend, database) the acceptance run installs, reconfigures and asserts per-tier scoping on, `morzer config list/get/set/unset` as a journaled operation that re-creates rather than restarts (a restart would leave a changed port in place, proved by an acceptance stage), a `doctor` check for an `installation.yaml` edit that decides nothing, plus declarations, `init --set`, delivery to Compose/templates/hooks, ports and health URLs that follow the value, and a Compose subprocess that no longer inherits the operator's environment — the acceptance run now installs on a non-default port and asserts the deployment answers there. |
