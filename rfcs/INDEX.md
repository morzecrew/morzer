...
|---|---|---|---|
| [0001](0001-update-and-rollback.md) | Update and rollback | 🚧 In progress | Adds `update` and `rollback` as engine operations: fetch, verify against a recorded digest, gate on `compatibility`, take a pre-update backup, swap the release pointer, and compensate by reverting it. Wires the already-written but uncalled `CheckUpgrade`/`AssessRollback` — P1–P2 shipped: both functions under test (which found two defects), a second bundle fixture, and `update` with fault injection at six points. `rollback` remains. Never rolls a database back automatically — the assessment reports containers, schema and restore-required separately, and refuses rather than guesses. |
...
| [0005](0005-continuous-integration-and-release.md) | Continuous integration and release | 🚧 In progress | Adds `.github/` with a change-gated `quality → test → coverage → acceptance` topology ported from forze, invoking `just` recipes so local and CI cannot drift. P1 shipped: quality, test and the skip guard. Fails the build when a real-adapter contract suite *skips*, which today lets CI pass without ever exercising the production secret store. Tagged releases call CI as a reusable workflow; actions are SHA-pinned. |
